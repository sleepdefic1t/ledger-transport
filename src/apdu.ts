...
export class Apdu {
    ...
    public async send(transport: LedgerTransport<any>): Promise<Buffer> {
        let promises: Buffer[] = [];
    ...
    private getChunks(payload: Buffer): Buffer[] {
        return payload
            .toString("hex")
            .match(new RegExp(`.{1,${this.CHUNK_SIZE * 2}}`, "g"))!
...
//     private getChunksFromPayload(payload: Buffer): Buffer[] {
...
//             } else if (index === chunks.length - 1 && chunks.length > 1) {
//                 /** Nth where N > 1 */
//                 p1 = ApduFlags.P1_LAST;
//             }

//             /** send the Apdu chunk */
//             promises.push(await transport.send(this.cla, this.ins, p1, this.p2, chunk));
//             index += 1;
//         }
